...
async function seed() {
  ...
  for (const account of accounts) {
    ...
    console.log("Seeding for account:", accountId);
    ...
    for (const item of SA_PARTS_SEED) {
      // Try to update first
      ...
      if (!updated || updated.length === 0) {
        ...
        await supabase.from('parts_catalog').insert(item);
      }
    }
  }

  console.log(`Successfully seeded for all accounts.`);
}
